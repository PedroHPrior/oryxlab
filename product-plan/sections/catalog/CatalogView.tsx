    ...
    >
      ...
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1">
            ...
            <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
              <ul>
                {filtered.map((item) => (
                  ...
                  >
                    <ItemSprite spriteId={item.sprite} itemType={item.type} rarity={item.rarity} size="sm" />
                    <div className="min-w-0 flex-1">
          ...
          {filtered.length === 0 && viewMode !== "sets" && (
      ...
      {quickCompare.open && (
        <div
          ...
          onClick={onCloseQuickCompare}
        >
          <div
            className="w-full rounded-t-2xl border border-zinc-200 bg-white p-5 shadow-2xl dark:border-zinc-800 dark:bg-zinc-900"
            onClick={(e) => e.stopPropagation()}
            style={{ maxHeight: "70vh" }}
          >
            <header className="mb-3 flex items-center justify-between">
              <h2 className="text-sm font-semibold">
                Quick compare ({quickCompare.selectedIds.length})
              </h2>
              <button
                type="button"
                onClick={onCloseQuickCompare}
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
              {quickCompare.selectedIds.map((id) => {
                const item = itemMap.get(id)
                if (!item) return null
                return <ItemCard key={id} item={item} onOpenDetail={() => onOpenItemDetail?.(id)} />
              })}
            </div>
          </div>
        </div>
