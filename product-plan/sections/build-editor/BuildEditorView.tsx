              ...
              {SLOTS.map(({ key, label }) => {
                ...
                return (
                  <li key={key}>
                    ...
                    >
                      {item ? (
                        <ItemSprite spriteId={item.sprite} itemType={item.type} rarity={item.rarity} size="lg" />
                      ) : (
        ...
        <div className="flex flex-col gap-4">
          <section className="grid grid-cols-2 gap-4 rounded-xl border border-zinc-200 bg-white p-5 sm:grid-cols-3 lg:grid-cols-4 dark:border-zinc-800 dark:bg-zinc-900">
            <div className="col-span-2 sm:col-span-3 lg:col-span-4">
              <Stat label="DPS" value={ds.dps.toLocaleString()} size="xl" />
              <div className="mt-1 flex items-center gap-3 text-[11px] text-zinc-500">
                <span>at Def {build.scenario.targetDefense}</span>
                <span>·</span>
                <span>DPS@0def {ds.dpsAtZeroDef.toLocaleString()}</span>
                <span>·</span>
                <span>TTK 1k HP: {ds.timeToKill1k.toFixed(2)}s</span>
              </div>
            </div>
            <Stat label="EHP" value={ds.ehp.toLocaleString()} size="md" />
