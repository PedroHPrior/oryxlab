...
export function ItemCard({
  ...
  return (
    <article
      className={`oryx-virtual-card group relative flex flex-col gap-2 rounded-xl border bg-white p-3 transition-all dark:bg-zinc-900 ${
        selected
...
function effectiveRoF(item: CatalogItem): number | null {
  const s = item.stats
  if (typeof s.shotsPerSec === "number") {
    return s.shotsPerSec * (s.rateOfFireMod ?? 1)
  }
  const base = item.weaponType ? WEAPON_BASE_ROF[item.weaponType] : undefined
  if (typeof base !== "number") return null
  return base * (s.rateOfFireMod ?? 1)
}

function keyStats(item: CatalogItem): { label: string; value: string }[] {
  const s = item.stats
  if (item.type === "weapon" && s.dmgMin && s.dmgMax) {
    const rof = effectiveRoF(item)
    return [
      { label: "Dmg", value: `${s.dmgMin}–${s.dmgMax}` },
      { label: "RoF", value: rof != null ? `${rof.toFixed(2)}/s` : "?/s" },
